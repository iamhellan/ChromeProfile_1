...
// Закрываем возможный popup
            closeResetPasswordPopupIfVisible();
            closeIdentificationPopupIfVisible();
            page.waitForTimeout(1000);

            // --- После идентификации открывается ЛК ---
            try {
                System.out.println("Открываем Личный кабинет...");
                page.waitForSelector("a.header-lk-box-link[title='Личный кабинет']",
                        new Page.WaitForSelectorOptions().setTimeout(5000));
                page.click("a.header-lk-box-link[title='Личный кабинет']");
                page.waitForTimeout(1000);
            } catch (Exception e) {
                System.out.println("⚠ Не удалось открыть Личный кабинет (возможно, другое состояние): " + e.getMessage());
            }
